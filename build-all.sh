...
compileAll()
{
	...
	for package in $(ls "$INIT_DIR/workdir"); do
		...
		if [ -f "$INIT_DIR/built-pkgs/$package-$pkgVersion-$ARCHITECTURE.zip" ]; then
			...
			if [ ! -d "$INIT_DIR/workdir/$package/destdir-pkg/data/data/com.vectras.winux" ]; then
				echo "- Package: '"$package"' failed to compile. Check logs"
				exit 0
			fi
...
case $* in "--16kb")
	echo "Warning: Compiling Winux RootFS with experimental support to 16kb pagesizes, Work is not garanted"
	echo ""
